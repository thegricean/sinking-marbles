...
ggplot(r, aes(x=Prior, y=response, color=Quantifier)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~ProportionBin)
ggsave(file="graphs/quantifier_byprior_byproportion.pdf",height=8,width=10)

agrr = aggregate(response ~ Prior + ProportionBin + Quantifier + Combination,data=r,FUN=mean)
...
ggsave(file="graphs/norm_means_ub_annotated.pdf",height=20,width=27)
