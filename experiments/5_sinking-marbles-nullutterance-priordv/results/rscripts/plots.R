...
# by-participant responses to "some" as function of prior
some = subset(r, quantifier == "Some")
some = droplevels(some)
ggplot(some, aes(x=Prior,y=response)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~workerid)
ggsave("graphs/some-by-subject-variation.pdf",width=100,height=10)
