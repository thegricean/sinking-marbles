...
setwd("~/cogsci/projects/stanford/projects/sinking_marbles/sinking-marbles/experiments/3_sinking-marbles-nullutterance/results/")
...
agrr$CILow = aggregate(normresponse ~ Prior.3 + Proportion + quantifier + Combination,data=r, FUN=ci.low)$normresponse
agrr$CIHigh = aggregate(normresponse ~ Prior.3 + Proportion + quantifier + Combination,data=r,FUN=ci.high)$normresponse
...
nrow(some)
summary(some)
m = lmer(normresponse ~ Prior.3 + (1|workerid) + (1|Combination),data=some)
summary(m)
library(lmerTest)
