...
function make_slides(f) {
  ...
  slides.cause_effect_prior = slide({
	...
	var pronoun = '';
	if (stim.other_gender == "M") {
		pronoun = "he";
	} else {
		pronoun = "she";
	}
      var trial_objnumber = getRandomInt(0, stim.N + 1);
      var description = stim.other_name + " observed that "+ trial_objnumber + " of the " + stim.object + " " + stim.effect + ". "+ caps(pronoun) +
       " describes what "+pronoun+ " observed to a friend:";
      var prompt = "Please write down two expressions that could fill in the blank. An expression can be more than one word, if you think that's most appropriate.";

      this.stim.actual_utterance =  '"________ of the ' + stim.object +
                      ' ' + stim.effect + '."';                      
    ...
    button : function() {
      ...
      if (ok_to_go_on) {
	    this.stim.word1 = word1;
	    this.stim.word2 = word2;	    
        this.log_responses();
        $(".err").hide();
    ...
    log_responses : function() {
        exp.data_trials.push({
		  ...
		  "word1": this.stim.word1,
		  "word2": this.stim.word2,
...
//      "quantifier": quantifiers[i],//_.shuffle(quantifiers)[0],
  ...
  exp.structure=["i0", "instructions", "practicetrial","cause_effect_prior", 'subj_info', 'thanks'];
