  ...
  slides.cause_effect_prior = slide({
    ...
    present_handle : function(stim) {
      ...
      if (stim.end_cause.length > 0) {
        var cause = stim.name + " " + stim.beginning_cause + " <span class='highlight_numbers'>" + stim.N.toString() + "</span> " + stim.object + " " + stim.end_cause + ".";
      } else {
      ...
      $("#upper_half").html(Math.ceil((N+1)/2).toString() + " to " + (N-1).toString() + " " + stim.object);
      $("#cause").html(cause);
...
function init() {
  ...
  function makeStim(i) {
    ...
    return {
      "name": name,
      "quantifier": _.shuffle(quantifiers)[0],
      "N": _.shuffle(Ns)[0],
      "name": name,
      "gender": gender,
