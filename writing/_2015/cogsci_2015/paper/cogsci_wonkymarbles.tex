...
Exp.~1 measured listeners' prior beliefs about how many objects would exhibit a certain effect (e.g., how many marbles sink).%, $P(s)$.
...
$F_{u_{\textrm{some}}}(s) {=} \delta_{s>0}$, \red{should this really be a delta?}
...
\begin{figure}
	...
	\caption{For each item, rRSA and wRSA model predicted $\mathbb{E}[P(s|u_{\textrm{some}})]$ as a function of $\mathbb{E}[P(s)$].}
...
Exp.~2a measured participants' posterior beliefs $p(s|u)$ in how many objects exhibited a certain effect (e.g., marbles sinking), after observing an utterance. 
...
Participants read the same descriptions as in Exp.~1. They additionally saw an utterance produced by a knowledgeable speaker about the event, e.g.~\textit{John, who observed what happened, said: ``Some of the marbles sank''}, and, as in Exp.~1, were asked to provide a judgment of an effect, e.g.~\textit{How many of the marbles do you think sank?}, on a sliding scale from 0 to 15.

 Each participant saw 10 \emph{some} trials and 20 fillers, of which 10 contained the quantifiers \emph{all} or ``none'', and the rest were utterances that did not address the number of objects that displayed the effect, e.g.~\textit{What a stupid thing to do.} The utterances were randomly paired with 30 random items for each participant.

 
 \subsubsection{Results and discussion}
 \red{XXX question}

The mean number of objects judged to exhibit the effect increased with increasing expectation of the prior distribution ($\beta$=.18, $SE$=.02, $t$=7.4, $p$$<$.0001, see also \figref{fig:meanresponses}).



, replicating the effect observed in Exp.~2a. Again, the effect of the prior was much smaller than predicted by rRSA and resulted in mean proportions of affected objects between 30\% and 65\%, where rRSA predicts a range from \red{XXX} to \red{XXX} for these items.
...
The procedure and materials were identical to those of Exp.~2a with the exception of the dependent measure. Rather than providing estimates of how many objects exhibited an effect, they  rated on sliding scales with endpoints labeled ``very unlikely'' and ``very likely'', how likely they thought 0\%, 1-50\%, 51-99\%, or 100\% of the objects exhibited the effect.
