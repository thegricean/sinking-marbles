...
Generalizing our opening example, take the case of \emph{Some of the X-s Y-ed} (for category \emph{X} and event \emph{Y}). When the prior probability, $\theta_{X,Y}$, of an X Y-ing is not extreme, RSA leads to the standard scalar implicature: the posterior probability that all $15$ of the Xs Yed, after hearing the utterance, is much lower than its prior probability. This is because a rational speaker would have been expected to say the more informative \emph{all of the X-s Y-ed} if it had been true.
...
In Experiment 3 we explore participants' intuitions about whether the world is normal in the scenarios of Experiment 2 and find that wRSA predicts listeners' wonkiness judgements. \red{make sure this follows the new organization}
...
Data from one participant, who gave only one response throughout the experiment, were excluded. Each item received between 12 and 29 ratings. Distributions of ratings for each item were smoothed using \red{Laplace smoothing}.  %nonparametric density estimation for ordinal categorical variables \cite{liracine2003} using the \verb|np| package in R \cite{hayfield2013} \red{see hayfield 2013 np package specification for li and racine ref --- or laplace if that's what we use.}. 
As intended, items covered a wide range of probabilities. \red{include measure or plot of this once you decide which smoothing to use ultimately.}
...
We see that the prior has a strong effect, which can be summarized by the two predictions described in the Introduction.\footnote{\red{comment on the noise?}} We next turn to an empirical test of these predictions, or rather, of the intuition that they may be incorrect.
...
To capture the idea that the pragmatic listener is unsure what background knowledge the speaker is bringing to the conversation, we extend the basic RSA model by using a ``lifted variable'' \cite{goodmanlassiter,lassitergoodman,bergen,kao} corresponding to the choice of state prior. That is, we posit that the prior, now $P(s|w)$, depends on a ``wonkiness'' variable $w$, which determines if it is the ``usual'' prior for this domain or a more generic back-off prior that we take to be uniform. The same prior is used in the literal and pragmatic listener, indicating that it is taken to be common ground. However, the $w$ variable is reasoned about only by the pragmatic listener, which captures the idea that it is an inference the pragmatic listener makes about which communication system is relevant. Using the notation of the earlier modeling section:
\begin{eqnarray}
&&P(s|w) = \begin{cases}
...
To make predictions for Exp.~2 from wRSA we use the smoothed empirical priors from Exp.~1 as $P_{\text{usual}}(s)$ for each item. The wonkiness prior $P(w)$ and the speaker optimality $\lambda$ are fit to optimize \red{mean squared error} with Exp.~2 data. 
\red{point to figure, describe fit to exp 2 data. comparison to rRSA}
...
One could imagine various approaches to engineer this departure\footnote{Though the authors tried several, which all failed by maintaining one or the other of the qualitative predictions of RSA identified in the introduction.\red{we should give examples}}, is this actually because listeners are inferring from an utterance like ``some of the marbles sank'' that the world is unusual?
...
Qualitatively, the model captures both the linear increase and decrease in wonkiness probability for \emph{all} and \emph{none}, respectively. Importantly, it also captures the asymmetric U-shaped wonkiness probability curve displayed by \emph{some}. \red{brief explanation why}. For comparison to the comprehension data fit, the correlation between model predictions and empirical wonkiness probabilities was $r = $\red{XXX}, with normalized root-mean-squared-error of \red{XXX}. \red{one sentence summarizing difference to comprehension data or move to the discussion?}
