...
That is, if the utterance is odd, the listener will revise her opinion about what world knowledge is appropriate to use to reconstruct the speaker's meaning. %\red{we need to explain where oddness comes from -- maybe there's an easy graphical way of showing how for items with different priors, a `some' utterance is more or less wonky, by showing the marginal probabilities of observing each utterance for these different items?}

\blue{Figure X shows the wRSA predicted number of objects as a result of a ``Some'' utterance. wRSA does not exhibit the same asymptotic behavior as RSA. As the prior probability of the event increases, wRSA predicts a dampened inference with respect to the number of objects with the property and the implication is not expected to cancel as the base rate approaches 1. These predictions qualitatively match the data observed in Expt. 2}

The quantitative predictions of wRSA depend upon the model's two free parameters: the speaker optimality parameter $\lambda$ in Eq. \ref{eq:S1w} and the prior probability of wonkiness $P(w)$ in Eq. \ref{eq:L1w}.
\mht{perform a single model BDA to infer the parameters of wRSA to report MSE and r-squared}
\mht{describe how we analyze the model using the ``Some'' data filtering out the 0s... but that the results are comparable if you add a noise process to account for the 0-responses}
...
Wonky RSA is a more complex model than regular RSA because of the additional degree of freedom in the wonkiness prior. 
To account for the additional flexibility of wRSA and our uncertainty about the values of the free parameters, we perform a Bayesian model comparison between these models by estimating the marginal likelihood of the data under each model using Annealed Importance Sampling \cite{neal2001annealed}. 
The Bayes Factor comparing wonky RSA to regular RSA is on the order of $e^{1500}$ in favor of wonky RSA.
The data strongly argue for the existence a wonkiness parameter that allows listeners the opportunity to revise common ground when utterances are odd.
