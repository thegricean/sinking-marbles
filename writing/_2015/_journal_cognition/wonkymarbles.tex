...
As we will show in \sectionref{sec:rosa}, RSA makes two strong predictions about the effect of the prior: 
...
In Exp.~1 we collect prior probabilities for a variety of events (e.g., sinking) and categories that participate in those events (e.g., marbles). In Exps.~2a and 2b we collect corresponding posterior interpretations after observing descriptions of those events containing quantifiers (\emph{some}, \emph{all}, \emph{none}). These experimental results confirm the intuition of relatively strong implicature---hence prediction (1) of RSA is incorrect---and show that the prior has a muted effect on posterior expectation---hence prediction (2) of RSA is incorrect.
...
The extended wRSA model makes predictions for listeners' judgments of world wonkiness. In Exp.~3 we explore participants' intuitions about whether the world is normal or wonky in the scenarios of Exps.~2a and 2b and find that wRSA predicts listeners' wonkiness judgments.

\red{Finally, we address the possibility that it is not the perceived wonkiness of the \emph{world}, but of the \emph{speaker}, that is driving the attenuated effect of the prior. In Exp.~4 we show that speaker reliability affects listeners' interpretations, but not in the way that would be expected if speaker reliability was driving the attenuated effect, providing further support for the idea that it is indeed the revision of prior beliefs about the world that is responsible for the muted effect of prior beliefs on utterance interpretation. We discuss these results with respect to a) ideal adaptor models in other areas of psycholinguistics and b) presupposition accommodation / common ground update.}


\section{Experiment 1: prior elicitation} 
\label{sec:priors}

Exp.~1\footnote{This experiment can be viewed at \url{http://cocolab.stanford.edu/cogsci2015/wonky/prior/sinking-marbles-prior.html}} measured listeners' prior beliefs about how many objects exhibit a certain effect (e.g., how many marbles sink).%, $P(s)$.
...
RSA is defined by: \red{XXX say that the $\delta$ is the Kronecker $\delta$ with one sentence of explanation}
\begin{eqnarray}
...
That is, if the utterance is odd, the listener will revise her opinion about what world knowledge is appropriate to use. \red{XXX I think we really need to explain to people where oddness comes from -- maybe there's an easy graphical way of showing how for items with different priors, a `some' utterance is more or less wonky, by showing the marginal probabilities of observing each utterance for these different items?}
...
\red{Thus far, we have only considered the possibility that it is the wonkiness of the event or the objects involved in the event that drive the attenuated effect of the prior on listeners' interpretations. However, an alternative possibility is that listeners believe that the speaker is an unreliable or uncooperative speaker who talks about the world in surprising and unexpected ways. Idea: arrgh; how could speaker wonkiness possibly get you an attenuated effect??. ... This makes the following prediction: if speaker reliability is explicitly manipulated, listeners should be more likely to follow rRSA in their interpretation of \emph{Some of the X Yed} when the speaker is unreliable but wRSA when the speaker is reliable. That is, we expect a replication of Exps.~2a and 2b for reliable, but not unreliable speakers. If, instead, the attenuated effect of the prior on comprehension is driven by listeners' perception of the speaker as unreliable, manipulating speaker reliability should have no effect on how strongly the prior influences listeners' interpretations. If anything, explicitly unreliable speakers should further diminish the effect of the prior.}
