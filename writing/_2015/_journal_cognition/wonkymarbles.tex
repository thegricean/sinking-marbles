...
Obtaining good estimates of prior beliefs is crucial for testing any Bayesian model. Indeed, \red{cite marcus/love for people who hate on Bayesians for being lax in their treatment of priors}. A problem for us, then, is that there are no clearly established dependent measures for eliciting prior beliefs about different types of events. One way to solve this problem is to elicit priors using different dependent measures and using the data obtained in this way to infer the underlying `true' prior. This is what we do here. Exp.~1a and 1b\footnote{These experiments can be viewed at \url{http://cocolab.stanford.edu/cogsci2015/wonky/prior/sinking-marbles-prior.html} and \red{XXX insert slider measure experiment}} measured listeners' prior beliefs about how many objects exhibit a certain effect (e.g., how many marbles sink) using two different dependent measures. The `true' prior that is used in all subsequent analyses throughout the paper was then inferred by \red{XXX}.

\subsection{Method}

\subsubsection{Participants}
For each experiment, we recruited 60 participants over Amazon's crowd-sourcing platform Mechanical Turk. Participants were paid \red{XXx} 2\$ (Exp.~1b) for their participation. Here and in all other experiments reported in this paper, participants' IP address was limited to US addresses only and only participants with a past work approval rate of at least 95\% were accepted.
...
\subsubsection{Participants}
For each experiment we recruited 120 participants over Amazon's Mechanical Turk who were paid \red{XXX} for their participation.
...
For utterances of \emph{Some of the X Yed}, the mean number of objects judged to exhibit the effect increased with increasing expectation of the prior distribution ($\beta$=.18, $SE$=.02, $t$=7.4, $p$$<$.0001). Similarly, the probability of all 15 objects exhibiting the effect increased with increasing prior probability of doing so ($\beta$=.06, $SE$=.01, $t$=5.0, $p$$<$.0001). However, the size of these effects is, to say the least, much smaller than predicted by RSA (for comparison, see dark lines in \figref{fig:rrsaexppredictions}).% that is, \citeA{geurts2010}'s observation that implicatures are very strong even for very likely events is confirmed empirically.
...
\subsubsection{Participants}
We recruited 60 participants over Amazon's crowd-sourcing platform Mechanical Turk who were paid \red{XXX} for their participation.
...
Some readers may still not be convinced that what is driving the attenuated effect of the prior is that listeners revise their beliefs about the world. Thus far, we have only considered the possibility that it is the wonkiness of the event or the objects involved in the event that drive the attenuated effect of the prior on listeners' interpretations. However, an alternative possibility is that listeners believe that the speaker is an unreliable or uncooperative speaker who talks about the world in surprising and unexpected ways. For example, maybe participants in the studies reported thus far believed that the speaker was a) lying, b) not trying to be informative, c) or not actually knowledgeable about the precise state of the world, i.e., about how many objects displayed the effect.

There are multiple reasons why we believe this is not a plausible explanation for our results. First, participants were happy to interpret the speaker's utterances literally in the \emph{all} and \emph{none} conditions, suggesting they didn't believe the speaker was lying, ruling out a). Second, the wonkiness judgments obtained in Exp.~3 closely match the wRSA model's predictions, which relies on speaker reliability and cooperativity (knowing the actual state of the world and reporting it truthfully and informatively), ruling out b) and c). However, there is an even better reason for ruling out speaker reliability explanations for the observed attenuated effect of the prior: if the speaker was actually unreliable, the model predicts that listeners should use their prior world knowledge much more strongly than if the speaker was reliable. That is, speaker unreliability predicts a much smaller reduction in the effect of the prior than empirically observed. 

\red{To see this, XXX discuss effect of setting lambda to 0: random choice between between true utterances) put actual model predictions in a plot}


This makes the following prediction: if speaker reliability is explicitly manipulated, listeners' interpretations of \emph{Some of the X Yed} should be much more strongly governed by their prior beliefs when the speaker is unreliable/uncooperative than when the speaker is reliable/cooperative. That is, we expect a replication of Exp.~2b for reliable, but not unreliable speakers. If, instead, the RSA models are fundamentally wrong and the attenuated effect of the prior on comprehension is driven by listeners' perception of the speaker as unreliable, further decreasing speaker reliability should have no effect on how strongly the prior influences listeners' interpretations. If anything, explicitly unreliable speakers should further diminish the effect of the prior. We test the prediction about speaker reliability in Exp.~4.
...
\begin{figure}
	...
	\caption{For each item, empirical posterior $P(s_{15}|u_{\textrm{some}})$ against  prior $P(s_{15})$ from Exp.~1, separately for reliable (\emph{sober}) and unreliable (\emph{drunk, court}) speakers.}
	\label{fig:speakerreliability}	
\end{figure}

Mean posterior $P(s_{15}|u_{\textrm{some}})$ ratings for the different speaker reliability conditions are shown in \figref{fig:speakerreliability}. While the reliable (\emph{sober}) speaker condition tracks the results from Exp.~2b very closely, the unreliable speaker conditions (\emph{court, drunk}) display a much greater effect of prior beliefs on the posterior probability of the all-state. This is borne out in a linear mixed effects regression model predicting the posterior all-state probability from centered fixed effects of prior all-state probability, speaker reliability, and their interaction \red{random effects maximal by-item and by-participant random effects structure}. Speaker reliability was coded as a binary variable (reliable vs.~unreliable) and centered. 

There was a main effect of prior all-state probability such that the posterior all-state probability increased with prior all-state probability ($\beta$=.26, $SE$=.03, $t$=8.5, $p$$<$.0001). There was also a main effect of speaker reliability, such that posterior all-state probability was judged higher when the speaker was unreliable ($\beta$=.11, $SE$=.02, $t$=7.4, $p$$<$.0001). Finally, there was an interaction between prior all-state probability and speaker reliability such that the slope of the prior all-state probability effect was shallower for the reliable speaker than for the unreliable speakers ($\beta$=.14, $SE$=.04, $t$=4, $p$$<$.0001).

\red{This suggests that XXX}
