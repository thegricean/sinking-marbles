...
plotname <- paste("phiPosterior-",prefix, mhiter/1000, "kBurn", burn/1000, "k.pdf", sep='')
...
ggplot(d.mixed, aes(x=MAPpredictive, y = freq, color=factor(bins)))+
  geom_point()+
 # geom_errorbarh(aes(xmin =credLow, xmax=credHigh))+
  ...
  ylim(-0.05,1.05)+
  coord_fixed()+
  ggtitle(paste("r2 = ",round(with(d.mixed, cor(freq, MAPpredictive))^2, digits=4),
                "\n MSE = ", round(with(d.mixed, mse(freq, MAPpredictive)),digits=4), sep=''))



plotname <- paste("scatter-modelData",prefix, mhiter/1000, "kBurn", burn/1000, "k.pdf", sep='')
ggsave(file=paste(fpath,"figures/", plotname, sep=''), 
       width = 20, height = 20)
