...
fname = '../results/model_results/results_uniform_sliderpriors.txt.tmp.results'
file_names = [fname]
...
headers = ["Item","QUD","Wonky","State","Alternatives","WonkyWorldPrior","Quantifier","SpeakerOptimality","Prior","PosteriorProbability"]        
k = 0
wwcnt = -1
condcnt = 0
priorcnt = 0
...
#		print k
		for j,pa in enumerate(pairs):
			...
			ssize = pasplit[1]
			prob = probs[j]
			results.append([item,qud, wonky, ssize, alts, wonkyworldprior, quantifier, spopt, prior,prob])
		k = k + 1
		priorcnt = priorcnt + 1		
...
oname = '../results/data/parsed_uniform_priorslider_results.tsv'
