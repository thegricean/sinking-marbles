#' ---
#' title: "Prior probabilities in the interpretation of 'some': analysis of uniform_nullutterance prior wonky world model predictions"
#' author: "Judith Degen"
...
load("data/mp-uniform_nullutterance.RData")
d = read.table("data/parsed_uniform_nullutterance_results.tsv", quote="", sep="\t", header=T)
table(d$Item)
nrow(d)
head(d)
...
mp = ddply(d, .(Item, QUD, State, Alternatives, Quantifier, SpeakerOptimality, WonkyWorldPrior,NullUtteranceCost), summarise, PosteriorProbability=sum(PosteriorProbability))
head(mp)
#mp[mp$Item == "ate the seeds birds" & mp$QUD=="how-many" & mp$Alternatives=="0_basic" & mp$SpeakerOptimality == 1,]
wr = ddply(d, .(Item, QUD, Wonky, Alternatives, Quantifier, SpeakerOptimality, WonkyWorldPrior,NullUtteranceCost), summarise, PosteriorProbability=sum(PosteriorProbability))
...
some =  ddply(msome, .(Item, QUD, Alternatives, SpeakerOptimality, PriorExpectation, Proportion, WonkyWorldPrior, NullUtteranceCost, PosteriorProbability_empirical), summarise, PosteriorProbability_predicted=sum(PosteriorProbability), PriorProbability_smoothed=sum(PriorProbability))
nrow(some)
head(some)
...
toplot = droplevels(subset(msome, QUD == "how-many" & SpeakerOptimality == 2 & Alternatives == "0_basic" & NullUtteranceCost == 2))#"0_basic1_lownum2_extra4_twowords5_threewords"))
nrow(toplot)
...
ggsave("graphs/model-empirical-uniform_nullutterance-howmany-2-basic.pdf",width=35,height=30)
...
pexpectations = ddply(mp[mp$Quantifier == "some",], .(Item, QUD, Alternatives, SpeakerOptimality,PriorExpectation_smoothed, WonkyWorldPrior, NullUtteranceCost, PosteriorExpectation_empirical), summarise, PosteriorExpectation_predicted=sum(State*PosteriorProbability)/15)
head(pexpectations)
some = pexpectations
...
head(cors)
some = droplevels(subset(some, QUD=="how-many"))
...
#  geom_text(data=cors, aes(label=r)) +
  #scale_size_discrete(range=c(1,2)) +
  #scale_color_manual(values=c("red","blue","black")) +
  facet_grid(NullUtteranceCost~Alternatives)
ggsave("graphs/model-empirical-uniform_nullutterance-expectations.pdf",width=30,height=10)

  
#plot empirical against predicted allstate-prbabilities for "some"
allstate = droplevels(subset(mp, State == 15 & Quantifier == "some" & QUD=="how-many"))
...
ggplot(allstate, aes(x=PosteriorProbability, y=PosteriorProbability_empirical,color=as.factor(WonkyWorldPrior), shape=as.factor(SpeakerOptimality))) +
  ...
  #scale_color_manual(values=c("red","blue","black")) +
  facet_grid(NullUtteranceCost~Alternatives)
ggsave("graphs/model-empirical-uniform_nullutterance-allstateprobs.pdf",width=30,height=10)
...
ggplot(allstate, aes(x=PriorProbability, y=PosteriorProbability,color=as.factor(WonkyWorldPrior), shape=as.factor(SpeakerOptimality))) +
  ...
  #scale_color_manual(values=c("red","blue","black")) +
  facet_grid(NullUtteranceCost~Alternatives)
ggsave("graphs/model-uniform_nullutterance-allstateprobs.pdf",width=30,height=10)
...
ggplot(toplot, aes(x=PriorExpectation, y=PosteriorProbability, color=Quantifier)) +
  geom_point() +
  geom_smooth() +
  facet_grid(QUD~NullUtteranceCost)
ggsave(file="graphs/wonkinessplot.pdf",width=6)

cors = ddply(wonky, .(Alternatives, QUD, SpeakerOptimality, Quantifier, WonkyWorldPrior,NullUtteranceCost), summarise, r=cor(PosteriorProbability, PosteriorProbability_empirical))
cors = cors[order(cors[,c("r")],decreasing=T),]
head(cors,15)


wonky_all = subset(wonky, Quantifier == "all" & QUD == "how-many")
wonky_none = subset(wonky, Quantifier == "none" &  QUD == "how-many")
wonky_some = subset(wonky, Quantifier == "some" &  QUD == "how-many")
...
ggplot(wonky_all, aes(x=PosteriorProbability, y=PosteriorProbability_empirical, color=as.factor(WonkyWorldPrior), shape=as.factor(SpeakerOptimality))) +
  ...
  scale_y_continuous(limits=c(0,1)) +  
  facet_grid(NullUtteranceCost~Alternatives)
ggsave("graphs/model-empirical-uniform_nullutterance-wonkiness_all.pdf", width=30,height=10)
...
ggplot(wonky_none, aes(x=PosteriorProbability, y=PosteriorProbability_empirical, color=as.factor(WonkyWorldPrior), shape=as.factor(SpeakerOptimality))) +
  ...
  scale_y_continuous(limits=c(0,1)) +  
  facet_grid(NullUtteranceCost~Alternatives)
ggsave("graphs/model-empirical-uniform_nullutterance-wonkiness_none.pdf", width=30,height=10)
...
ggplot(wonky_some, aes(x=PosteriorProbability, y=PosteriorProbability_empirical, color=as.factor(WonkyWorldPrior), shape=as.factor(SpeakerOptimality))) +
  ...
  scale_y_continuous(limits=c(0,1)) +  
  facet_grid(NullUtteranceCost~Alternatives)
ggsave("graphs/model-empirical-uniform_nullutterance-wonkiness_some.pdf", width=30,height=10)
...
ggplot(wonky_all, aes(x=PriorExpectation, y=PosteriorProbability, color=as.factor(WonkyWorldPrior), shape=as.factor(SpeakerOptimality))) +
  ...
  scale_y_continuous(limits=c(0,1)) +  
  facet_grid(NullUtteranceCost~Alternatives)
ggsave("graphs/model-uniform_nullutterance-all-wonkiness.pdf", width=30,height=10)
...
ggplot(wonky_none, aes(x=PriorExpectation, y=PosteriorProbability, color=as.factor(WonkyWorldPrior), shape=as.factor(SpeakerOptimality))) +
  ...
  scale_y_continuous(limits=c(0,1)) +  
  facet_grid(NullUtteranceCost~Alternatives)  
ggsave("graphs/model-uniform_nullutterance-none-wonkiness.pdf", width=30,height=10)
...
ggplot(wonky_some, aes(x=PriorExpectation, y=PosteriorProbability, color=as.factor(WonkyWorldPrior), shape=as.factor(SpeakerOptimality))) +
  ...
  scale_y_continuous(limits=c(0,1)) +  
  facet_grid(NullUtteranceCost~Alternatives) 
ggsave("graphs/model-uniform_nullutterance-some-wonkiness.pdf", width=15,height=10)

save(mp, file="data/mp-uniform_nullutterance.RData")
save(wr, file="data/wr-uniform_nullutterance.RData")
...
ggsave("~/cogsci/conferences_talks/_2015/2_cogsci_pasadena/wonky_marbles/paper/pics/model-expectations-uniform_nullutterance.pdf",width=5.5,height=4.5)
...
# plot both rRSA and uniform_nullutterance wRSA expectation predictions in same plot
...
#scale_color_manual(values=c("red","blue","black")) 
#ggsave("graphs/model-expectations.pdf",width=5.5,height=4.5)#,width=30,height=10)
ggsave("~/cogsci/conferences_talks/_2015/2_cogsci_pasadena/wonky_marbles/paper/pics/model-expectations-uniform_nullutterance-regular.pdf",width=6.5,height=4.5)
