...
load("data/mp-uniform-prits.RData")
d = read.table("data/parsed_uniform_prits_results.tsv", quote="", sep="\t", header=T)
table(d$Item)
nrow(d)
head(d)
summary(d)
...
priorexpectations = read.table(file="/Users/titlis/cogsci/projects/stanford/projects/thegricean_sinking-marbles/bayesian_data_analysis/ordered_expectations.txt",sep="\t", header=T, quote="")
priorexpectations$Item = as.factor(gsub("cd players","cd-players",as.character(priorexpectations$Item)))
...
priorprobs = read.table(file="/Users/titlis/cogsci/projects/stanford/projects/thegricean_sinking-marbles/bayesian_data_analysis/prits_withnames.txt",sep="\t", header=T, quote="")
head(priorprobs)
priorprobs$Item = as.factor(gsub("cd players","cd-players",as.character(priorprobs$Item)))
...
ggsave("graphs/4_prits/model-empirical-uniform-howmany-2-basic-prits.pdf",width=35,height=30)
...
#ggsave(file="graphs/4_prits/model-empirical-diff-by-confidence-annotated.pdf",width=20,height=16)
ggsave(file="graphs/4_prits/model-empirical-diff-by-confidence.pdf",width=10,height=8)
...
ggsave(file="graphs/4_prits/model-empirical-diff-by-confidence-annotated.pdf",width=20,height=16)
#ggsave(file="graphs/4_prits/model-empirical-absdiff-by-confidence.pdf",width=10,height=8)
...
some[,c("Item","WonkyWorldPrior","SpeakerOptimality","Predicted","Empirical")] %>% 
  save(file="data/some-expectations-prits.RData")
...
ggsave("graphs/4_prits/model-empirical-uniform-expectations-prits.pdf",width=20,height=10)
...
ggplot(some, aes(x=PriorExpectation_smoothed, y=PosteriorExpectation_predicted,color=as.factor(SpeakerOptimality), shape=as.factor(WonkyWorldPrior))) +
  ...
  #scale_color_manual(values=c("red","blue","black")) +
  facet_grid(SpeakerOptimality~WonkyWorldPrior)
ggsave("graphs/4_prits/model-uniform-expectations-prits.pdf",width=20,height=10)
...
test = ddply(allstate, .(SpeakerOptimality, WonkyWorldPrior), summarise, mse=round(gof(PosteriorProbability, PosteriorProbability_empirical)["MSE",],3),r=gof(PosteriorProbability, PosteriorProbability_empirical)["r",],R2=gof(PosteriorProbability, PosteriorProbability_empirical)["R2",])
test = test[order(test[,c("mse")]),]
head(test,10)
test = test[order(test[,c("r")],decreasing=T),]
head(test,10)
...
ggsave(file="graphs/4_prits/optimal-uniform-prits.pdf")
...
ggplot(allstate, aes(x=PosteriorProbability, y=PosteriorProbability_empirical,color=as.factor(WonkyWorldPrior), shape=as.factor(SpeakerOptimality))) +
  ...
  #scale_color_manual(values=c("red","blue","black")) +
  facet_grid(SpeakerOptimality~WonkyWorldPrior)
ggsave("graphs/4_prits/model-empirical-uniform-allstateprobs-prits.pdf",width=30,height=10)

# plot by-item diff in predicted and empirical expectation by confidence rating
confidence = read.table("../../../experiments/24_sinking-marbles-prior-prits/results/data/confidence.txt",sep="\t",quote="",header=T)
nrow(confidence)
head(confidence)
row.names(confidence) = confidence$Item
...
#geom_text(aes(label=Item))
#ggsave(file="graphs/4_prits/model-empirical-diff-by-confidence-annotated.pdf",width=20,height=16)
ggsave(file="graphs/4_prits/model-empirical-allprobs-diff-by-confidence.pdf",width=10,height=8)
...
#geom_text(aes(label=Item))
#ggsave(file="graphs/4_prits/model-empirical-diff-by-confidence-annotated.pdf",width=20,height=16)
ggsave(file="graphs/4_prits/model-empirical-allprobs-absdiff-by-confidence.pdf",width=10,height=8)
...
ggplot(allstate, aes(x=PriorProbability, y=PosteriorProbability,color=as.factor(WonkyWorldPrior), shape=as.factor(SpeakerOptimality))) +
  ...
  #scale_color_manual(values=c("red","blue","black")) +
  facet_grid(SpeakerOptimality~WonkyWorldPrior)
ggsave("graphs/4_prits/model-uniform-allstateprobs-prits.pdf",width=30,height=10)
...
head(wonky)
toplot = droplevels(subset(wonky,  WonkyWorldPrior == .5 & SpeakerOptimality == 0))

ggplot(toplot, aes(x=PriorExpectation, y=PosteriorProbability, color=Quantifier)) +
  geom_point() +
  geom_smooth() 
ggsave(file="graphs/4_prits/wonkinessplot-prits.pdf",width=6)
...
ggplot(wonky_all, aes(x=PosteriorProbability, y=PosteriorProbability_empirical, color=as.factor(WonkyWorldPrior), shape=as.factor(SpeakerOptimality))) +
  ...
  scale_y_continuous(limits=c(0,1)) +  
  facet_grid(WonkyWorldPrior~SpeakerOptimality)
ggsave("graphs/4_prits/model-empirical-uniform-wonkiness_all-prits.pdf", width=30,height=10)
...
ggplot(wonky_none, aes(x=PosteriorProbability, y=PosteriorProbability_empirical, color=as.factor(WonkyWorldPrior), shape=as.factor(SpeakerOptimality))) +
  ...
  scale_y_continuous(limits=c(0,1)) +  
  facet_grid(WonkyWorldPrior~SpeakerOptimality)
ggsave("graphs/4_prits/model-empirical-uniform-wonkiness_none-prits.pdf", width=30,height=10)
...
ggsave("graphs/4_prits/model-empirical-uniform-wonkiness_some-prits.pdf", width=30,height=10)
...
ggplot(wonky, aes(x=Confidence,y=abs(PredictionDiff),color=Quantifier)) +
  ...
  geom_smooth() +
  facet_grid(WonkyWorldPrior~SpeakerOptimality)
ggsave("graphs/4_prits/model-empirical-uniform-wonkiness-prits-abs-confidence.pdf", width=12,height=10)

ggplot(wonky, aes(x=Confidence,y=PredictionDiff,color=Quantifier)) +
  geom_point() +
  geom_smooth() +
  facet_grid(WonkyWorldPrior~SpeakerOptimality)
ggsave("graphs/4_prits/model-empirical-uniform-wonkiness-prits-confidence.pdf", width=12,height=10)
...
ggplot(wonky_all, aes(x=Confidence,y=PredictionDiff)) +
  ...
  geom_hline(yintercept=0) +
  facet_grid(WonkyWorldPrior~SpeakerOptimality)
ggsave("graphs/4_prits/model-empirical-uniform-wonkiness-all-prits-confidence.pdf", width=12,height=10)
...
ggplot(wonky_all, aes(x=Confidence,y=abs(PredictionDiff))) +
  ...
  geom_smooth() +
  facet_grid(WonkyWorldPrior~SpeakerOptimality)
ggsave("graphs/4_prits/model-empirical-uniform-wonkiness-all-prits-abs-confidence.pdf", width=12,height=10)
...
ggplot(wonky_some, aes(x=Confidence,y=PredictionDiff)) +
  ...
  geom_hline(yintercept=0) +
  facet_grid(WonkyWorldPrior~SpeakerOptimality)
ggsave("graphs/4_prits/model-empirical-uniform-wonkiness-some-prits-confidence.pdf", width=12,height=10)

ggplot(wonky_some, aes(x=Confidence,y=abs(PredictionDiff))) +
  geom_point() +
  geom_smooth() +
  facet_grid(WonkyWorldPrior~SpeakerOptimality)
ggsave("graphs/4_prits/model-empirical-uniform-wonkiness-some-prits-abs-confidence.pdf", width=12,height=10)
...
ggplot(wonky_none, aes(x=Confidence,y=PredictionDiff)) +
  ...
  geom_hline(yintercept=0) +
  facet_grid(WonkyWorldPrior~SpeakerOptimality)
ggsave("graphs/4_prits/model-empirical-uniform-wonkiness-none-prits-confidence.pdf", width=12,height=10)

ggplot(wonky_none, aes(x=Confidence,y=abs(PredictionDiff))) +
  geom_point() +
  geom_smooth() +
  facet_grid(WonkyWorldPrior~SpeakerOptimality)
ggsave("graphs/4_prits/model-empirical-uniform-wonkiness-none-prits-abs-confidence.pdf", width=12,height=10)

save(mp, file="data/mp-uniform-prits.RData")
save(wr, file="data/wr-uniform-prits.RData")
