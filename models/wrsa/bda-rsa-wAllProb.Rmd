...
When do BDA on RSA / wRSA, we are only going to observe the "comp_state" data ('how many marbles sank?').
...
var dataAnalysis = function(){
  ...
  foreach(items, function(i){
    ...
    query.add(["allprob", i], probability(15, l1State))
...
md.wPrior <- left_join(
  bind_rows(post.pred %>% filter(param == "state") %>%
            rename(response = MAP, ci_lower = credLo, ci_upper = credHi) %>% 
              ungroup() %>%
              select(-param) %>% mutate(src = "model"),
          d.summ %>% rename(response = mean) %>% ungroup() %>%
...
## Wonky RSA 2: `wonky = flip()`, `backoff = binomial(0.5, ..)`

Same as before, but a binomial(0.5) backoff prior as opposed to a uniform.
...
save(bda.results,
      file = "models/wrsa/results/wrsa2_binomBackoff_mcmc1kb0.5k_x2.Rdata")
```

```{r}
load("~/Documents/research/sinking-marbles/models/wrsa/results/wrsa2_binomBackoff_mcmc1kb0.5k_x2.Rdata")
qplot(bda.results$`alpha,NA`)
...
ggplot(mdWonky.wPrior, aes( x = priorExpectation, y = response, ymin = ci_lower, ymax = ci_upper ))+
  geom_point()+
  facet_wrap(~src)+
  geom_errorbar(alpha = 0.1)+
  geom_smooth()
...
## Wonky RSA 3: `wonky = flip()`, `backoff = binomial(p, ..)`


Same as v2, but infer the binomial weight (as opposed to binomial(0.5))
...
var backoffPrior = function(p){
  return map(function(b){
    return probability(b, Binomial({p:p, n:15}))
  }, Binomial({p:p, n:15}).support())
}
...
// pragmatic listener
var pragmaticListener = function(utt, alpha, backoffWeight, wonkyPrior, itemPrior) {
  return Infer({method:"enumerate"},
  function(){
    var wonky = flip(wonkyPrior);
    var prior = wonky ? backoffPrior(backoffWeight) : itemPrior
    var state = domainPrior(prior)
    observe(speaker(state, alpha, prior), utt)
    return {state: state, wonky: wonky}
  })
...
var dataAnalysis = function(){
  ...
  //var wonkyPrior = uniformDrift({a:0, b:1, width: 0.1});
  var wonkyPrior = 0.5;
  var backoffWeight = uniformDrift({a:0, b:1, width: 0.1});
  ...
  foreach(items, function(i){
    ...
    var l1Predictions = pragmaticListener("some", alpha, backoffWeight, wonkyPrior, itemPrior);
  ...
  query.add(["alpha", "NA"] , alpha)
  query.add(["wonkyPrior", "NA"] , wonkyPrior)
  query.add(["backoffWeight", "NA"] , backoffWeight)
...
save(bda.results,
      file = "models/wrsa/results/wrsa3_binomBackoff_inferP_mcmc1kb0.5k_x2.Rdata")
```

```{r}
load("~/Documents/research/sinking-marbles/models/wrsa/results/wrsa3_binomBackoff_inferP_mcmc1kb0.5k_x2.Rdata")
qplot(bda.results$`alpha,NA`)
...
## Wonky RSA 4: `wonky = flip(q)`, `backoff = uniform(...)`

We now try a wonky RSA model with a wonkyPrior probability that we will infer from the data, and a uniform back off prior.
...
save(bda.results,
     file = "models/wrsa/results/wrsa1_inferQ_mcmc200b100k_x2.Rdata")
```

### Posterior on alpha

```{r}
load("~/Documents/research/sinking-marbles/models/wrsa/results/wrsa1_inferQ_mcmc200b100k_x2.Rdata")
qplot(bda.results$`alpha,NA`)
...
### Posterior on wonky probability

```{r}
qplot(bda.results$`wonkyPrior,NA`)
```



Infers a very high wonkiness prior probability.
...
Behavior here still looks pretty good.
...
Same asymmetric-U, but super compressed.

## Wonky RSA 5: `wonky = flip(q)`, `backoff = binomial(0.5, ..)`
...
save(bda.results,
      file = "models/wrsa/results/wrsa2_inferQ_binomBackoff_mcmc200b100k_x2.Rdata")
```

```{r}
load("~/Documents/research/sinking-marbles/models/wrsa/results/wrsa2_inferQ_binomBackoff_mcmc200b100k_x2.Rdata")
qplot(bda.results$`alpha,NA`)
...
## Wonky RSA 6: `wonky = flip(q)`, `backoff = binomial(p, ..)`


Same as v5, but infer the binomial weight (as opposed to binomial(0.5))
...
save(bda.results,
      file = "models/wrsa/results/wrsa3_inferQ_binomBackoff_inferP_mcmc200b100_x2.Rdata")
```

```{r}
load("~/Documents/research/sinking-marbles/models/wrsa/results/wrsa3_inferQ_binomBackoff_inferP_mcmc200b100_x2.Rdata")
qplot(bda.results$`alpha,NA`)
